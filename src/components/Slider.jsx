...
const Slider = () => {
    ...
    const { pokemonData, pokemonDetailsAll, pokemonDetailsFilter, pokemonSliderSelection, setPokemonSliderSelection, pokemonFilters, sliderPage, setSliderPage, pokemonPerSlide } = useContext(AppContext);


    // const pokemonDetailsAllLocal = localstorage.key('pokemon');
    ...
    console.log("arrivedData", pokemonDetailsFilter)
    return (
        <div className='h-[24rem] overflow-hidden'>
            <div className="px-32 pt-10 mx-auto w-full flex flex-wrap justify-center">
                ...
                {pokemonData
                    ...
                    .map((pok) => (pokemonSliderSelection[pok.id - 1] &&
                        <div key={pok.id} className="m-5 pokemonGalleryItem rounded-full">
                            <img src={`${pokemonSliderSelection[pok.id - 1].sprites.front_default}`} width="130px" />
                        </div>)
                    )}
